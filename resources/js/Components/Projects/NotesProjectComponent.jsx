  ...
  return (
   ...
   <div className="p-6 bg-white rounded-lg shadow-md m-6 space-y-2">
    ...
    {filteredData.map((entry, index) => (
      <div key={index} className="p-6 bg-white rounded-lg shadow-md m-6 space-y-2">
  ...
  <div className="flex items-center">
    ...
    <div className="flex items-center mx-4">
      <div className="w-24 h-2 bg-red-300 mr-2 rounded">
        <div
          className={`h-2 rounded ${
            entry.status >= 50 ? 'bg-green-500' : 'bg-red-500'
          }`}
      ...
      <span className="text-sm font-semibold">{entry.status}%</span>
        ...
        <h3 className="text-lg font-semibold">  {entry.firstname} {entry.lastname}</h3>
        <h3 className="text-base font-semibold">{entry.department}</h3>
        <div className="p-4 bg-gray-100 rounded-lg">
      <p className="mt-2 text-sm">{formatTextWithLineBreaks(entry.note)}</p>
    </div>
      </div>
    ))}
  </div>
